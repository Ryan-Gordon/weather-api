...
def query_metric(sensor_ids: list, metrics: list, statistic='average', date_range='latest'):
    ...
    if date_range == 'latest':
        ...
        start_date, end_date = map(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d'), date_range.split(':'))
    ...
    for m in metrics:
        ...
        if values:
            if statistic == 'min':
                ...
                result[m] = sum(values) / len(values)
        else:
            result[m] = None  # or some default value or message indicating no data found
