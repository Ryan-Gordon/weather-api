...
def populate_sensor_data():
    with connex_app.app.app_context():
        for i in range(10):
            ...
            temp_value = round(random.uniform(18.0, 28.0), 2)  # random values between 18 and 28
            humidity_value = round(random.uniform(40, 60), 2)  # random values between 40% and 60%
            ...
            # Create data entries
            temp_data = SensorData(sensor_id="sensor-1", metric="temperature", value=temp_value, unit_of_measurement="degrees:celsius", timestamp=date)
            humidity_data = SensorData(sensor_id="sensor-1", metric="humidity", value=humidity_value, unit_of_measurement="percent", timestamp=date)
...
def request_sensor_data(client_fixture, sensor_id):
    ...
    response = client_fixture.get(f"/api/query?sensor_ids={sensor_id}&metrics=temperature,humidity&statistic=average")
    return response
